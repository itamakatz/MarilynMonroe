 ...
 * M356 - Print Fan speeds. Created by Itamar Katz
...
inline void gcode_M356() {
  for (int p = 0; p < FAN_COUNT; ++p) {
    SERIAL_ECHO_START();
    SERIAL_ECHOLNPAIR("Speed of Fan Number: ", (unsigned long)p);
    SERIAL_ECHO(fanSpeeds[p]);
    SERIAL_EOL();
  }
}
...
void process_parsed_command() {
  ...
  switch (parser.command_letter) {
    ...
    case 'M': switch (parser.codenum) {
      ...
      case 356: // M356 Print Fan speeds. Created by Itamar Katz
        gcode_M356();
        break;
